...
Value getstakesubsidy(const Array& params, bool fHelp)
{
    ...
    return (uint64_t)GetProofOfStakeReward(pindexBest, nCoinAge, 0, nHeight);
}
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Agora is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Agora is downloading blocks...");
